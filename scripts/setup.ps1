...
try {
    ...
    $ffVersion = ($ffOut | Select-Object -First 1).ToString()
...
$nvidiaPresent = $false
try {
    $smiOut = & nvidia-smi --query-gpu=name --format=csv,noheader 2>&1
...
Write-Host "Run: python scripts/download_models.py"
Write-Host "Then: dotnet build frontend/SubtitleCreator"
Write-Host ""
